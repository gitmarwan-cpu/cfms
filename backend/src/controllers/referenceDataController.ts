...
const listAllLists = catchAsync(async (req: AppRequest, res: AppResponse) => res.status(200).json({ success: true, data: await referenceDataService.getAllLists(req.organizationId) }));
...
const updateItem = catchAsync(async (req: AppRequest, res: AppResponse) => res.status(200).json({ success: true, message: 'تم تحديث العنصر بنجاح', data: await referenceDataService.updateItem(req.params.key, req.organizationId, req.params.itemId, req.body) }));
