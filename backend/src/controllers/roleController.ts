...
const createRole = catchAsync(async (req: AppRequest, res: AppResponse) => res.status(201).json({ success: true, message: 'تم إنشاء الدور بنجاح', data: await roleService.createRole(req.organizationId, req.body) }));
const updateRole = catchAsync(async (req: AppRequest, res: AppResponse) => res.status(200).json({ success: true, message: 'تم تحديث الدور بنجاح', data: await roleService.updateRole(req.organizationId, req.params.id, req.body) }));
const deleteRole = catchAsync(async (req: AppRequest, res: AppResponse) => { await roleService.deleteRole(req.organizationId, req.params.id); res.status(200).json({ success: true, message: 'تم حذف الدور بنجاح' }); });
