...
const assignRole = catchAsync(async (req: AppRequest, res: AppResponse) => res.status(201).json({ success: true, message: 'تم إسناد الدور بنجاح', data: await userRoleService.assignRole(req.organizationId, { userId: req.params.userId, roleId: req.body.roleId, orgUnitId: req.body.orgUnitId }) }));
