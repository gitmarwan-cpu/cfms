...
const mapComplainant = (
  ...
  return {
    ...
    createdAt: complainant.created_at,
    updatedAt: complainant.updated_at,
...
const mapComplaint = (complaint: ComplaintRecord, referenceItems: Map<number, ReferenceItem>) => {
  ...
  const result: Record<string, unknown> = {
    ...
    createdByUserId: complaint.created_by_user_id,
    type: complaint.type,
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const complaint = await tx.complaints.create({
      data: {
        ...
        created_by_user_id: parsedCreatedByUserId,
        type: payload.type,
      ...
      await tx.complaint_attachments.createMany({
        data: files.map((file) => {
          ...
          return {
            ...
            created_at: attachmentNow,
            updated_at: attachmentNow,
          };
        }),
      });
    }
