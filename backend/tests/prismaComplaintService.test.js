...
describe('Prisma complaint service', () => {
  ...
  it('searches complaints by complainant name, phone, and description text', async () => {
    await complaintService.createComplaint(
      organization.id,
      makePayload(locations, {
        isAnonymous: false,
        fullName: 'Searchable Beneficiary Name',
        phone: '778000001',
        description: 'A complaint about transportation services and school bus delays.',
      })
    );
    await complaintService.createComplaint(
      organization.id,
      makePayload(locations, {
        isAnonymous: false,
        fullName: 'Unrelated Citizen',
        phone: '778000002',
        description: 'Water quality issue at the distribution point.',
      })
    );

    const byName = await complaintService.listComplaints(organization.id, { search: 'Searchable Beneficiary' });
    expect(byName.data).toHaveLength(1);
    expect(byName.data[0].complainant.fullName).toBe('Searchable Beneficiary Name');

    const byPhone = await complaintService.listComplaints(organization.id, { search: '778000002' });
    expect(byPhone.data).toHaveLength(1);
    expect(byPhone.data[0].complainant.fullName).toBe('Unrelated Citizen');

    const byDescription = await complaintService.listComplaints(organization.id, { search: 'school bus' });
    expect(byDescription.data).toHaveLength(1);

    const noMatch = await complaintService.listComplaints(organization.id, { search: 'nonexistent-search-term' });
    expect(noMatch.data).toHaveLength(0);
  });

  it('keeps complaint search tenant-isolated', async () => {
    const otherOrganization = await makeOrganization('search-other');
    await complaintService.createComplaint(
      otherOrganization.id,
      makePayload(locations, {
        isAnonymous: false,
        fullName: 'CrossTenantSearchTarget',
        phone: '778000003',
      })
    );

    const results = await complaintService.listComplaints(organization.id, { search: 'CrossTenantSearchTarget' });
    expect(results.data).toHaveLength(0);

    const foreignResults = await complaintService.listComplaints(otherOrganization.id, { search: 'Searchable Beneficiary' });
    expect(foreignResults.data).toHaveLength(0);
  });

  it('filters complaints by priority code', async () => {
    const created = await complaintService.createComplaint(
      organization.id,
      makePayload(locations, { isSensitive: true, priority: 'high' })
    );
    const filtered = await complaintService.listComplaints(organization.id, { priority: 'high' });
    expect(filtered.data.some((complaint) => complaint.id === created.complaint.id)).toBe(true);

    const notLow = await complaintService.listComplaints(organization.id, { priority: 'low' });
    expect(notLow.data.some((complaint) => complaint.id === created.complaint.id)).toBe(false);
  });
});

describe('New complaint notifications', () => {
  let locations;
  ...
  it('notifies active users with complaints.view_all in the same organization', async () => {
    const org = await makeOrganization('notify');
    const admin = await setup.createUserWithRole({
      fullName: 'Notify Admin',
      email: `notify-admin-${Date.now()}@cfms.local`,
      roleCode: 'admin',
    ...
    const result = await complaintService.createComplaint(
      org.id,
      makePayload(locations, { isAnonymous: false, fullName: 'Notified Citizen', phone: '778000010' })
    );

    const notifications = await prisma.notifications.findMany({
      where: { organization_id: org.id, entity_id: result.complaint.id },
    });
    ...
    expect(notifications[0]).toMatchObject({
      ...
      entity_id: result.complaint.id,
      read_at: null,
    });
    expect(notifications[0].message).toContain(result.complaint.referenceCode);
  });

  it('does not notify staff with view_own only, nor admins of other organizations', async () => {
    const org = await makeOrganization('notify-other');
    await setup.createUserWithRole({
      fullName: 'Notify Staff',
      email: `notify-staff-${Date.now()}@cfms.local`,
      roleCode: 'staff',
      organizationId: org.id,
    });

    const result = await complaintService.createComplaint(org.id, makePayload(locations));
    ...
    const foreignOrg = await makeOrganization('notify-foreign');
    const foreignAdmin = await setup.createUserWithRole({
      fullName: 'Foreign Admin',
      email: `foreign-admin-${Date.now()}@cfms.local`,
      roleCode: 'admin',
      organizationId: foreignOrg.id,
    });
    const foreignNotifications = await prisma.notifications.findMany({
      where: { user_id: foreignAdmin.user.id, entity_id: result.complaint.id },
    });
    expect(foreignNotifications).toHaveLength(0);
  });

  it('does not notify deactivated users', async () => {
    const org = await makeOrganization('notify-inactive');
    const admin = await setup.createUserWithRole({
      fullName: 'Inactive Admin',
      email: `inactive-admin-${Date.now()}@cfms.local`,
      roleCode: 'admin',
      organizationId: org.id,
    });
    await prisma.users.update({ where: { id: admin.user.id }, data: { is_active: false } });
    await prisma.user_organizations.updateMany({
      where: { user_id: admin.user.id, organization_id: org.id },
